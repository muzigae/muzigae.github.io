...
먼저, 풀이의 흐름을 만들어보자.<br>
우리가 알아야 할 것은 어떤 섬 안에 들어있는 섬의 개수이다.<br>
지도를 탐색하면서 순서대로 섬에 번호를 붙인다고 생각해보자.<br>
어떤 섬이 다른 어떤 섬에 의해 둘러싸여 있다면, 바깥에 있는 섬이 먼저 탐색되기 때문에 분명히 그 섬은 자신을 둘러싼 섬보다 번호가 클 것이다.<br>
바로 이 점을 이용한다. 어떤 섬에서 바다를 탐색할 때 경계에 도달하지 않았다면 다른 섬 안에 있다는 뜻이 되고, 만날 수 있는 섬 중 가장 작은 번호의 섬이 자신을 둘러싼 섬이 된다.<br>
이렇게 각 섬마다 자신을 둘러싼 섬을 찾은 후 각 섬의 높이를 계산하면 끝난다.<br><br>
